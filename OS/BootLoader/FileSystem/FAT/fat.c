...
int fs_strncmp(const char * cs,const char * ct,size_t count)
{
...
int fs_strcmp(const char * cs,const char * ct)
{
...
fat_register_device(block_dev_desc_t *dev_desc, int part_no)
{
	...
	if(!fs_strncmp((char *)&buffer[DOS_FS_TYPE_OFFSET],"FAT",3)) {
				   ...
				   int dols)
{
	...
	for (i = 0; i < DIRENTSPERCLUST; i++) {
	    ...
	    if (fs_strcmp (filename, s_name) && fs_strcmp (filename, l_name)) {
	     ...
	     int dols)
{
	...
	mydata->fatlength = (__u16)bs.fat32_length;
    } else {
    ...
    fs_strcpy ((char *)fnamecopy, filename);
	...
	for (i = 0; i < DIRENTSPERBLOCK; i++) {
	    ...
	    if (fs_strcmp ((char *)fnamecopy, s_name) &&
					   fs_strcmp ((char *)fnamecopy, l_name)) {
