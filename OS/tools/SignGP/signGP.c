...
int main(int argc, char *argv[])
{
	...
	strcpy_s(ifname, _countof(ifname), "x-load.bin");
	loadaddr = 0x40200800;

	if ((argc == 2) || (argc == 3) || (argc == 4))
		strcpy_s(ifname, _countof(ifname), argv[1]);
	...
	strcat_s(ofname, _countof(ofname), ".ift");

	/* Open the input file. */
	ferr = fopen_s(&ifile, ifname, "rb");
	if (ferr || (ifile == NULL)) {
		err("Cannot open %s\n", ifname);
		pdie("fopen");
	}
