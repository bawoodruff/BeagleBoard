...
!  if exist($(RAZZLETOOLPATH)\configuration.inc) # in case DDKSPLIT does not work here
!    include $(RAZZLETOOLPATH)\configuration.inc
!  endif
!endif
# @@END_DDKSPLIT
...
# INTERNAL_SDK
# Win8 introduces INTERNAL_SDK_INC_PATH and INTERNAL_SDK_LIB_PATH as part
# of the "additional SDK separation". We dont want to port those changes
# to Win7 and our goal is to keep INETCORE codebase in sync; thus, we
# match INTERNAL_SDK_INC_PATH with SDK_INC_PATH and INTERNAL_SDK_LIB_PATH with SDK_LIB_PATH

!ifndef INTERNAL_SDK_INC_PATH
INTERNAL_SDK_INC_PATH = $(SDK_INC_PATH)
!endif

!ifndef INTERNAL_SDK_LIB_PATH
INTERNAL_SDK_LIB_PATH = $(SDK_LIB_PATH)
!endif
...
!if 0
# @@BEGIN_DDKSPLIT
_DEL=$(RAZZLETOOLPATH)\icmd.cmd del $@ $@.temp 2>nul
_DELFILE=$(RAZZLETOOLPATH)\icmd.cmd del
_MOVE=move $@.temp $@ >nul
_PERL=$(RAZZLETOOLPATH)\perl\bin\perl.exe
_TYPE=$(RAZZLETOOLPATH)\icmd.cmd type
_COPYFILE=$(RAZZLETOOLPATH)\icmd.cmd copy
_IF=$(COMSPEC) /c if
# @@END_DDKSPLIT
!endif
...
TARGET_BRACES=-B
TARGET_DEFINES=/D_PPC_ /DMPPC /D_XBOX /D_XBOX_VER=200 /D_XENON $(ENDIAN_DEFINES)
TARGET_DIRECTORY=ppc
PLATFORM_MFC_VER=0x0421

!ifdef SUBSTITUTE_PPC_CC
TARGET_CPP=$(SUBSTITUTE_PPC_CC)
!else
TARGET_CPP=cl.exe
!endif
...
!else
!error Must define the target as 386, ia64, amd64, arm or ppc.
!endif
